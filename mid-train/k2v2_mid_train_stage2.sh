#!/bin/bash
#SBATCH --job-name=k2v2-stage2-64k
#SBATCH --nodes=200
#SBATCH --ntasks=200
...
# ==========================================
TP=8
SAVE_ROOT="${YOUR_CKPT_PATH}/k2v2_stage2_attn64k_jais250k_tp${TP}_bestfit_fix"
...
TRAIN_DATA_PATH="${YOUR_DATA_PATH}/merged"

TOKENIZER_TYPE=HuggingFaceTokenizer
TOKENIZER_MODEL="${YOUR_TOKENIZER_PATH}/jais250k"

# Load from Stage 1 Checkpoint (Iteration 130000)
LOAD_CHECKPOINT_PATH="${YOUR_CKPT_PATH}/k2v2_stage1_attn8k_jais250k_tp8/checkpoints/checkpoint_0130000"
...
# K2V2 Architecture Configuration (Stage 2: 64k Context)
MODEL_ARGS=(
    --use-mcore-models
    --disable-bias-linear
    --seq-length 65536
    ...
    --rotary-base 1000000  # High RoPE base for long context
    --swiglu
...
# Training Hyperparameters (Stage 2: LR 6e-6, Constant, No Warmup)
TRAINING_ARGS=(
    --micro-batch-size 1
    --global-batch-size 200    # 200 nodes * 1 MBS / 1 CP = 200
    --seed 2
    --lr 6.0e-6
    --adam-beta1 0.9
    --adam-beta2 0.95
...
LOGGING_ARGS=(
    ...
    --wandb-project "k2v2_stage2"
    --wandb-exp-name "stage2-64k"
    --logging-level 20
)

# Checkpointing: Loading from Stage 1, Saving to Stage 2 dir
CHECKPOINT_ARGS=(
    --eval-interval 2500
    --eval-iters 40
    ...
    # Use this if you want to allow loading a checkpoint with different TP/seq-len configs
    # --no-load-rng
    # --no-load-optim
)
